...
# 4.匹配-诊断
# （1）查看是否每条记录都有关键词“小结”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('小结')]))  # 1191,说明有4条记录没有小结
# （2）提取“诊断”
renal_data['诊断'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.extract('.*?小结：([\d\D*|\D*]+)')
# （3）查看多少Nan值
print(renal_data[renal_data['诊断'].isna()])  # 4条记录为Nan,即医生有4条记录没有写小结
# （4）将Nan值填充为“无”
renal_data['诊断'] = renal_data['诊断'].fillna('无')

"""
去除“小结”部分
"""
# 5.去除DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)列中小结部分，防止总结中出现的描述与需要提取的关键词冲突
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.split('小结')[0])
# 6.再次检验DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)中是否存在小结
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('小结')]))  # 0
...
# 7.匹配-肾小球总数
# （1）查看共有多少条记录出现“未见肾小球”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('未见肾小球')]))   # 17 源代码结果是18？
# （2）“未见肾小球”17条记录，将“未见肾小球”修改为“0个未见肾小球”， 便于后续正则提取
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('未见肾小球', '0个未见肾小球'))
# （3）查看共有多少条记录出现“未见……肾小球”的书写情况
print(len(renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('(未见[\u4E00-\u9FA5]+肾小球)')))
# （4）提取肾小球总数
renal_data['肾小球总数'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.extract('[，。？！,.?!]*(\d+|[一二三四五六七八九十]+)\D*肾小球')
# （5）查看“肾小球总数”为Nan值的记录
print(renal_data[renal_data['肾小球总数'].isna()])
# 结果：显示仅有一条记录，内容为：皮髓质肾组织1条，仅见一个完全硬化的小球。（没有关键词“肾小球”）
# （6）将“肾小球总数”为Nan值的列根据实际情况填充值
renal_data.loc[804, '肾小球总数'] = '1'
# （7）检验“肾小球总数”为1、0的数据是否提取正确
print(renal_data[renal_data['肾小球总数'] == '1'])
# （8）数量不多，对应更正即可
renal_data.loc[523, '肾小球总数'] = '12'
renal_data.loc[737, '肾小球总数'] = '9'
print(renal_data[renal_data['肾小球总数'] == '0'])
# （9）“肾小球总数”列值转为int型数值
renal_data['肾小球总数'] = renal_data['肾小球总数'].astype(int)
"""
“球性硬化小球数”共有2种情况：
...
# 8.匹配--球性硬化小球数
# （1）查看共有多少条记录出现关键词“废弃”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('废弃')]))  # 615
# （2）查看共有多少条记录出现“未见……废弃”的书写情况
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('未见[\u4E00-\u9FA5]+废弃')]))  # 1
# （3）由于数量仅有1条，对应更正即可
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('未见球性废弃', '0个球性废弃'))
# （4）提取“球性硬化小球数”
renal_data['球性硬化小球数'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.extract('.*?(\d+)\D*废弃')
# （5）将Nan值填充为0
renal_data['球性硬化小球数'] = renal_data['球性硬化小球数'].fillna(0)
# （6）“球性硬化小球数”列值转为int型数值
renal_data['球性硬化小球数'] = renal_data['球性硬化小球数'].astype(int)
...
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('新月体')]))  # 252
# （2）查看共有多少条记录出现“未见新月体”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('未见新月体')]))  # 40
# （3）“未见新月体”40条记录，将“未见新月体”修改为“0个新月体”，便于后续正则提取
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('未见新月体', '0个新月体'))
# （4）查看“未见……新月体”的数量
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('未见[\u4E00-\u9FA5]+新月体')]))  # 4
# （5）由于数量仅有4条，且4种书写情况均不一致，则采用对应修正方式即可
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('未见确切新月体', '0个新月体'))
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('未见典型新月体', '0个新月体'))
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('未见袢坏死及新月体', '0个新月体'))
# （6）提取“新月体小球数目”
renal_data['新月体小球数目'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.extract('.*?(\d+)\D*新月体')
# （7）查看“新月体小球数目”列值是否有256个非空值
print(renal_data[renal_data['新月体小球数目'].notna()])  # 252
# （8）将Nan值填充为0
renal_data['新月体小球数目'] = renal_data['新月体小球数目'].fillna(0)
# （9）将“新月体小球数目”列值转为int型
renal_data['新月体小球数目'] = renal_data['新月体小球数目'].astype(int)
...
renal_data[['新月体1', '新月体2']] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.extract('.*?(\d+)\D*[纤维|细胞]\D*(\d+)\D*[纤维|细胞]\D*新月体')
# （11）将“新月体1”、“新月体2”两列的Nan值填充为0
renal_data[['新月体1', '新月体2']] = renal_data[['新月体1', '新月体2']].fillna(0)
# （12）将“新月体1”、“新月体2”两列的数值转换为int型
renal_data[['新月体1', '新月体2']] = renal_data[['新月体1', '新月体2']].astype(int)
# （13）将“新月体1”、“新月体2”两列的数值相加
renal_data['新月体12'] = renal_data['新月体1'] + renal_data['新月体2']
# （14）删除临时列“新月体1”、“新月体2”
renal_data = renal_data.drop(renal_data[['新月体1', '新月体2']], axis=1)
# （15）将“新月体12”列值与“新月体小球数目”列值进行对比，若“新月体12”列中的值大于“新月体小球数目”对应的值，则替换“新月体小球数目”列中的值，反之不替换。
renal_data['新月体小球数目'] = renal_data.apply(lambda x: max(x['新月体小球数目'], x['新月体12']), axis=1)
# （16）查看“新月体12”不为0的记录
print(renal_data[renal_data['新月体12'] != 0][['EMPI_ID', 'DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)', '新月体12']])  # 63
# （17）排查“新月体12”不为0的记录中有无第二次正则匹配错误的情况，若有则将其手动修改
renal_data.loc[74, '新月体小球数目'] = 1
# 皮髓质肾组织1条，5个肾小球中1个球性废弃，余正切球体积增大，系膜区节段中度增宽，系膜细胞增殖伴基质增多，毛细血管袢开放尚好，节段袢内皮细胞增殖、肿胀，袢内少量单个核细胞浸润，节段外周袢与囊壁粘连，1处见节段性细胞性新月体形成，囊壁节段增厚。
renal_data.loc[653, '新月体小球数目'] = 2
# "皮质及皮髓质肾组织2条。10个肾小球中9个球性废弃伴囊壁纤维素样渗出，且有2个废弃球可见纤维细胞性新月体残迹。
renal_data.loc[695, '新月体小球数目'] = 9
# 皮质及髓质肾组织2条，22个肾小球中见12个球性废弃，废弃球中见2个细胞性新月体残留，余10个肾小球见亦见2个细胞性及5个纤维细胞性新月体，受新月体挤压，其中6个球出现明显节段硬化，
renal_data.loc[844, '新月体小球数目'] = 3
# 皮髓质肾组织2条。10个肾小球中8个球已接近球性废弃，且均伴有明显的袢坏死、炎细胞浸润及囊壁断裂，其中3个球见纤维性新月体残迹。
renal_data.loc[1114, '新月体小球数目'] = 9
# 送检皮质肾组织2条，石蜡切片见7个肾小球，冰冻组织经石蜡包埋、切片后见10个肾小球，17个肾小球中见2个球性废弃、9个纤维细胞性新月体，并见1处节段瘢痕，内皮细胞未见明显增殖，但因新月体挤压致袢腔开放欠佳；
renal_data.loc[1132, '新月体小球数目'] = 19
# 皮髓质肾组织2条，19个肾小球，16个肾小球已废弃，大部分废弃球可见新月体残迹，剩余3个肾小球，其中2个球见细胞性新月体，1个球见纤维细胞性新月体，内皮细胞未见明显增殖，因新月体挤压几乎已无袢腔开放，系膜区增宽情况不易观察；
# （18）删除临时列“新月体12”
del renal_data['新月体12']
# （19）由于医生对新月体存在分开描述“包括5个纤维性、2个纤维细胞性及2个细胞性新月体（伴节段袢坏死），并见1处节段硬化，”，因此需要将三个数字都提取出来并算出总和替代“新月体小球数目”列只提取出的那个数值
renal_data[['新月体1', '新月体2', '新月体3']] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.extract('.*?(\d+)\D*[纤维|细胞]\D*(\d+)\D*[细胞|纤维]\D*(\d+)\D*[纤维|细胞]\D*新月体')
# （20）将“新月体1”、“新月体2”、“新月体3”三列的Nan值填充为0
renal_data[['新月体1', '新月体2', '新月体3']] = renal_data[['新月体1', '新月体2', '新月体3']].fillna(0)
# （21）将“新月体1”、“新月体2”、“新月体3”三列的数值转换为int型
renal_data[['新月体1', '新月体2', '新月体3']] = renal_data[['新月体1', '新月体2', '新月体3']].astype(int)
# （22）将“新月体1”、“新月体2”、“新月体3”三列的数值相加
renal_data['新月体123'] = renal_data['新月体1'] + renal_data['新月体2'] + renal_data['新月体3']
# （23）删除临时列“新月体1”、“新月体2”、“新月体3”
renal_data = renal_data.drop(renal_data[['新月体1', '新月体2', '新月体3']], axis=1)
# （24）查看“新月体123”不为0的记录
print(renal_data[renal_data['新月体123'] != 0][['EMPI_ID', 'DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)', '新月体123']])  # 14
# 检查结果都匹配第三次正则
# （25）将“新月体123”列值与“新月体小球数目”列值进行对比，若“新月体123”列中的值大于“新月体小球数目”对应的值，则替换“新月体小球数目”列中的值，反之不替换。
renal_data['新月体小球数目'] = renal_data.apply(lambda x: max(x['新月体小球数目'], x['新月体123']), axis=1)
# （26）删除临时列“新月体123”
del renal_data['新月体123']
...
# 10.匹配--节段性硬化数目
# （1）查看共有多少条记录出现关键词“硬化”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('硬化')]))  # 201
# （2）查看共有多少条记录出现关键词“节段性硬化”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('节段性硬化')]))  # 0
# （3）查看共有多少条记录出现关键词“节段硬化”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('节段硬化')]))  # 183
# （4）查看存在关键词“硬化”但非“节段硬化”的记录出现的描述
t1 = renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('硬化')]
print(t1[~t1['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('节段硬化')])  # 18
# （5）查看共有多少条记录出现关键词“无节段硬化”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('无节段硬化')]))  # 0
...
# （10）将Nan值填充为0
renal_data['节段性硬化数目'] = renal_data['节段性硬化数目'].fillna(0)
# （11）“球性硬化小球数”列值转为int型数值
renal_data['节段性硬化数目'] = renal_data['节段性硬化数目'].astype(int)
...
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('内皮细胞')]))  # 416
...
print(len(renal_data[renal_data['内皮细胞增生'].notna()]))  # 416
# （7）将Nan值填充为0
renal_data['内皮细胞增生'] = renal_data['内皮细胞增生'].fillna(0)
# （11）“球性硬化小球数”列值转为int型数值
renal_data['内皮细胞增生'] = renal_data['内皮细胞增生'].astype(int)
"""
# 总结：（内皮细胞增生、记录数量）
# 0    980
# 1    215
...
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('毛细血管袢')]))  # 1115
# （2）查看共有多少条记录出现关键词“毛细血管袢……开放”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('毛细血管袢[\u4E00-\u9FA5]+开放')]))  # 46
# （3）查看共有多少条记录出现关键词“毛细血管袢尚开放”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('尚开放')]))  # 46
# 注：出现关键词“毛细血管袢……开放”刚好46条记录均为“毛细血管袢尚开放”
# （4）查看共有多少条记录出现关键词“毛细血管袢……佳”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('开放[\u4E00-\u9FA5]+佳')]))  # 40
# （5）查看共有多少条记录出现关键词“毛细血管袢欠佳”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('开放欠佳')]))  # 40
# （6）查看共有多少条记录出现关键词“毛细血管袢不佳”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('开放不佳')]))  # 22
# （7）查看共有多少条记录出现关键词“毛细血管袢尚佳”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('开放尚佳')]))  # 4
# 注：出现关键词“毛细血管袢……佳”刚好40条记录包含了“毛细血管袢开放欠佳”、“毛细血管袢开放不佳”、“毛细血管袢开放尚佳”三种描述情况
# （8）将出现“开放欠佳”、“开放不佳”、“开欠佳”的记录修改为“开放差”
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('开放欠佳', '开放差'))  # 40
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('开放不佳', '开放差'))  # 22
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('开欠佳', '开放差'))  # 2
# （9）将存在“开放差”的记录确定为“毛细血管管腔”为1
renal_data.loc[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('开放差'), '毛细血管管腔'] = 1
# （10）将“毛细血管管腔”列为Nan值的记录改为0
renal_data['毛细血管管腔'] = renal_data['毛细血管管腔'].fillna(0)
"""
总结：（毛细血管管腔、记录数量）
0.0    1130
1.0      65
...
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('系膜区')]))  # 1167
# （2）提取包含关键词“系膜区”的记录，创建ximoqu表
ximoqu = renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('系膜区')][['EMPI_ID', 'DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)']]
...
renal_data = pd.merge(renal_data, ximoqu, on=['EMPI_ID', 'DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'], how='left')
# （9）“系膜区”的Nan值填充为0
renal_data['系膜区'] = renal_data['系膜区'].fillna(0)
# （10）删除“系膜区1”列
del renal_data['系膜区1']
# （11）“未见肾小球”无法提取系膜区信息，标记为“999”
renal_data.loc[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('未见肾小球'), '系膜区'] = 999
"""
总结：（系膜区、记录数量）
1      650
0      395
...
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('系膜细胞')]))  # 608
# （2）将存在“系膜细胞”的记录确定为“系膜细胞”为1
renal_data.loc[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('系膜细胞'), '系膜细胞'] = 1
# （3）将“系膜细胞”列为Nan值的记录改为0
renal_data['系膜细胞'] = renal_data['系膜细胞'].fillna(0)
"""
总结：（系膜细胞、记录数量）
1.0    608
0.0    587
...
# 15.匹配--系膜基质
# （1）查看共有多少条记录出现关键词“基质”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('基质')]))  # 643
# （2）将存在“基质”的记录确定为“系膜基质”为1
renal_data.loc[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('基质'), '系膜基质'] = 1
# （3）将“系膜基质”列为Nan值的记录改为0
renal_data['系膜基质'] = renal_data['系膜基质'].fillna(0)
"""
总结：（系膜基质、记录数量）
1.0    643
0.0    552
...
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('嗜复红物沉积')]))  # 805
# （2）查看共有多少条记录出现关键词“阴性”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('阴性')]))  # 354
# （3）查看共有多少条记录出现关键词“不确切”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('不确切')]))  # 1
# （4）查看共有多少记录出现关键词“嗜红物沉积”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('嗜红物沉积')]))  # 2
# （6）查看共有多少记录出现关键词“嗜复红物”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('嗜复红物')]))  # 813
# （7）查看共有多少记录出现关键词“嗜银”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('嗜银')]))  # 7
# （8）将出现关键词“阴性”的记录修改为“0免疫复合物”
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('阴性', '0免疫复合物，阴性'))
# （9）将出现内容为“大量嗜复红物沉积”的记录修改为“1免疫复合物”
renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'] = renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].apply(lambda x: x.replace('大量嗜复红物沉积', '0免疫复合物'))

print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧[\u4E00-\u9FA5]+嗜复红物沉积')]))  # 380  311
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧大量嗜复红物沉积')]))  # 66   0
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧少量嗜复红物沉积')]))  # 125
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧较多嗜复红物沉积')]))  # 113
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧较少嗜复红物沉积')]))  # 0
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧偶见嗜复红物沉积')]))  # 23
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及系膜区节段嗜复红物沉积')]))  # 2
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及系膜区少量嗜复红物沉积')]))  # 11
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及系膜区大量嗜复红物沉积')]))  # 1   0
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及系膜区较多嗜复红物沉积')]))  # 3
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及系膜区较少嗜复红物沉积')]))  # 0
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧节段嗜复红物沉积')]))  # 15
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧送检嗜复红物沉积')]))  # 1
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧疑似少量嗜复红物沉积')]))  # 2
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及基膜内少量嗜复红物沉积')]))  # 1
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及基膜内大量嗜复红物沉积')]))  # 2   0
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧弥漫嗜复红物沉积')]))  # 3
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧似见节段嗜复红物沉积')]))  # 1
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('上皮侧及内皮下少量嗜复红物沉积')]))  # 1   2
...
# 17.匹配--肾小管萎缩
# （1）查看共有多少条记录出现“小管萎缩”
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('小管萎缩')]))  # 1150
print(len(renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('萎缩')]))  # 1158
# （2）提取包含关键词“萎缩”的记录，创建weisuo表
weisuo = renal_data[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('萎缩')][['EMPI_ID', 'DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)']]
...
renal_data = pd.merge(renal_data, weisuo, on=['EMPI_ID', 'DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'], how='left')
# （12）将“萎缩不明显”记录的“肾小管萎缩”列值更改为0
renal_data.loc[renal_data['DBMS_LOB.SUBSTR(A.DIAG_DESC,4000)'].str.contains('萎缩不明显'), '肾小管萎缩'] = 0
# （13）将“肾小管萎缩”列值填充为0
renal_data['肾小管萎缩'] = renal_data['肾小管萎缩'].fillna(0)
"""
总结：（神小管萎缩、记录数量）
1.0    963
3.0    105
